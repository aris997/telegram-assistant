...
async def health_check(request: Request):
    logger.debug("GET Request OK 200")
...
async def messager(request: Request):
    ...
    message_type = [
        key for key in payload.get("message", {}).keys() if key not in UNUSEFUL_KEYS
    ]
        ...
        if not ECHO:
            ...
            completion = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Be an helpful assistant in a telegram chat. If it's needed use the right syntax to highlight titles, bullet points and more. You are allowed to use emojis.",
                    },
    ...
    message = {"chat_id": chat_id, "text": str(bot_message), "parse_mode": "MarkdownV2"}
